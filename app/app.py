...
async def recommend(
    ...
    calls recommend_agent.get_recommendation(), which acts as a genuine
    MCP client against *this same server's* MCP server object and lets an
    LLM (Groq) decide how to use the search_anime_manga_vibes tool, over
    an in-process transport (see recommend_agent.py's module docstring
    for why: a real network SSE round-trip to ourselves was reliably
    OOM-killing this process on Render's free tier). Requires
    `require_api_key` same as search (this route's own call) -- the
    per-key rate limit is enforced once, here, rather than again per
    internal tool call.
    """
    try:
        result = await recommend_agent.get_recommendation(vibe=request.vibe, media_type=request.type)
...
# this mount, so there's no collision. Note that /api/v1/recommend's
# agent connects to the `mcp` object directly in-process (not through
# this HTTP mount) -- see recommend_agent.py's module docstring for why.
#
